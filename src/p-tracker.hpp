...
class ptracker {
 public:
  ...
  int rotations{1};
  int progress{0};
  int optim_algo{POWELL};
  int powell_num_iter{5};
  ...
  // === FILES ===
  char image_name[256];  // Image name in "printf" style. Example:
                         // "./quelquepart/toto%04d.tif"
  char dic_name[256];    // DIC-file name in "printf" style. Example:
                         // "./quelquepart/dic_out_%d.txt"
  ...
  int rescue_level{2};  // in range [0 2].
                        // A negative value means no pixel-resolution tracking
                        // (go directly to subpixel optimization)
